...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        ...
        // 移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + wallSize.width * 2.5 + itemTexture.size().width)

        // 画面外まで移動するアクションを作成
        let moveItem = SKAction.moveBy(x: -movingDistance, y: 0, duration:4.771)
        ...
        // 出現位置の上下の振れ幅を鳥のサイズの5倍とする
        let random_y_range = birdSize.height * 5

        // アイテムのY軸を計算
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let item_lowest_y = center_y - random_y_range / 2
        // アイテムを生成するアクションを作成
            let createItemAnimation = SKAction.run({
                // アイテム関連のノードを乗せるノードを作成
                let item = SKNode()
                item.position = CGPoint(x: self.frame.size.width + wallSize.width * 3, y: 0)
                item.zPosition = -40 // 雲より手前、地面より奥、壁より手前
                ...
                itemApple.position = CGPoint(x: 0, y: item_y)
                
                // スプライトに物理演算を設定する
                itemApple.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
